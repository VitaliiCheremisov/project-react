def calculate_shopping_cart(ingredients):
    ...
    shopping_list = []
    max_length_name = max([len(ingredient['ingredients__name']) for ingredient in ingredients])
    max_length_amount = max([len(str(ingredient['amounts'])) for ingredient in ingredients])
